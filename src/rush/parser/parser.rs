use crate::rush::error::Response::Wrong;
use super::*;
        ...
        let statement = match self.current_type() {
            ...
            Symbol => match self.current_lexeme().as_str() {
                "@" => {
                    ...
                    Statement::new(StatementNode::Label(name), position)
                },
            ...
            Keyword => match self.current_lexeme().as_str() {
                ...
                "let" => {
                    ...
                    let right = if self.current_lexeme() == "\n" {
                        self.next_newline()?;
                        let body = self.parse_body()?;

                        if body.len() != 1 {
                            return Err(response!(
                                Wrong(format!("right-hand block should be a single expression")),
                                self.source.file,
                                position
                            ));
                        }

                        if let StatementNode::Expression(ref e) = body[0].node {
                            block_flag = true;
                            e.clone()
                        } else {
                            return Err(response!(
                    ...
                    let decl = Statement::new(StatementNode::Declaration(name, right), position);
                ...
                "goto" => {
                    ...
                    let station = self.parse_expression()?;

                    Statement::new(StatementNode::Goto(station), position)
                }
    ...
    fn parse_body(&mut self) -> Result<Vec<Statement>, ()> {
        ...
        if self.indent_standard == 0 {
            ...
            if self.indent % self.indent_standard != 0 {
                panic!();
                return Err(response!(
                    Wrong(format!("found inconsistently indentation: {} spaces", self.indent)),
                    self.source.file,
                ...
                Operator => match self.current_lexeme().as_str() {
                    "-" => {
                        ...
                        if self.current_lexeme() == "(" {
                            ...
                            Expression::new(
                                ExpressionNode::Neg(Rc::new(self.parse_atom()?)),
                                self.span_from(position),
                            )
                        }
                    }
                    ...
                    "+" => {
                        ...
                        if self.current_lexeme() == "(" {
                            Expression::new(
                                ExpressionNode::Abs(Rc::new(self.parse_expression()?)),
                                self.span_from(position),
                            )
                        } else {
                            Expression::new(
                                ExpressionNode::Abs(Rc::new(self.parse_atom()?)),
                                self.span_from(position),
                            )
                        }
                    }
